...
const formData = useForm({
  ...
  type: props.shipment.type,
    route: props.shipment.route
});

const handleEditShipment = () => {
    formData.put(route("shipments.update",{ shipment: props.shipment.id }), {
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 p-3 sm:flex gap-[2.75rem] ">
                <div class="mb-4">
                    <span>Editar Envío</span><br>
                </div>
                <div class="grow bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                        <form @submit.prevent="handleEditShipment()" method="POST">
                            ...
                            {{ formData.type }}
                            <div class="mt-4">
                                <InputLabel for="type" value="Tipo de Envío" />
                                <Dropdown v-model="formData.type" :options="typeShip" optionLabel="name" optionValue="value" placeholder="Selecciona Tipo" class="w-full md:w-14rem" />
                            </div>
